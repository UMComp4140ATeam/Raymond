...
class EvaluateLayerTest(unittest.TestCase):
    ...
    def test_process_add(self):
        ...
        self.assertEquals([0, 0, 0], ret[2].coefficient_vector.tolist())
    ...
    def test_process_mult(self):
        ...
        self.assertEquals([4, 2], ret[0].coefficient_vector.tolist())
        self.assertEquals(3, ret[0].ciphertext)
        self.assertEquals(1, ret[0].level)

class EvaluateCircuitTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.odd_modulus = 5
        cls.ciphertexts = [
            ciphertext.Ciphertext(numpy.array([1, 2], dtype=numpy.integer), 1, 0),
            ciphertext.Ciphertext(numpy.array([3, 2], dtype=numpy.integer), 2, 0),
            ciphertext.Ciphertext(numpy.array([1, 1], dtype=numpy.integer), 1, 0),
            ciphertext.Ciphertext(numpy.array([4, 1], dtype=numpy.integer), 2, 0),
            ciphertext.Ciphertext(numpy.array([0, 3], dtype=numpy.integer), 1, 0)
        ]
        g0 = [0, 1]
        ...
        """
        cls.circuit = evaluate.Circuit([l0, l1])
    ...
    def test_circuit(self):
        ...
        orig_size = len(self.ciphertexts)
        self.circuit.process(self.dimension, self.odd_modulus, self.ciphertexts, evaluation_key)
        ret = self.ciphertexts
        self.assertEquals(len(self.ciphertexts), orig_size + 5)
        self.assertEquals([0, 0], ret[7].coefficient_vector.tolist())
        self.assertEquals(4, ret[7].ciphertext)
        self.assertEquals(0, ret[7].level)
        self.assertEquals([4, 2], ret[8].coefficient_vector.tolist())
        self.assertEquals(3, ret[8].ciphertext)
        self.assertEquals(1, ret[8].level)
